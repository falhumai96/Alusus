...
def build_alusus(
    ...
    try:
        ...
        return False
    msg.info_msg(
        "Using vcpkg repo path={vcpkg_repo_path}.".format(
    ...
    environ[path_env_var] = (
        (vcpkg_repo_path + path_sep + environ[path_env_var])
        if path_env_var in environ
        else vcpkg_repo_path
    )
    ...
    cmake_cmd = [
        ...
        "-DALUSUS_BUILD_TESTS={build_tests}".format(
            build_tests=("ON" if build_tests else "OFF")
        ),
        "-DPython3_EXECUTABLE={python3_exec}".format(python3_exec=sys.executable),
    ]

    # APR does not include an official CMake package that can be located with "find_package". It uses pkg-config, which is only available on
    # Unix targets. Therefore, for Windows we have added manual configuration variables to be set from the CMake command line. For Unix targets,
    # we require pkg-config to be installed, which is the official auto-detect package configuration supported by APR.
    if alusus_target_triplet.value.platform == "win32":
        apr_include_dir = os.path.join(
            alusus_build_location,
            "vcpkg_installed",
            alusus_target_triplet.value.vcpkg_target_triplet,
            "include",
        )
        apr_compile_definitions = "APR_DECLARE_STATIC"  # We are linking APR statically.
        apr_lib_dir = os.path.join(
            alusus_build_location,
            "vcpkg_installed",
            alusus_target_triplet.value.vcpkg_target_triplet,
            "lib",
        )
        apr_libs = "Ws2_32;Rpcrt4"
        cmake_cmd += [
            "-DALUSUS_OSAL_APR_MANUAL_CONFIG=ON",
            "-DALUSUS_OSAL_APR_MANUAL_CONFIG_INCLUDE_DIR={apr_include_dir}".format(
                apr_include_dir=apr_include_dir
            ),
            "-DALUSUS_OSAL_APR_MANUAL_CONFIG_DEFS={apr_compile_definitions}".format(
                apr_compile_definitions=apr_compile_definitions
            ),
            "-DALUSUS_OSAL_APR_MANUAL_CONFIG_LIB_DIR={apr_lib_dir}".format(
                apr_lib_dir=apr_lib_dir
            ),
            "-DALUSUS_OSAL_APR_MANUAL_CONFIG_LIBS={apr_libs}".format(apr_libs=apr_libs),
        ]

    if force_cmake_generator:
        msg.info_msg(
            "Using forced {cmake_generator} CMake generator with the CMake configuration.".format(
                cmake_generator=json.dumps(force_cmake_generator)
            )
        )
        cmake_cmd += ["-G", force_cmake_generator]
    elif alusus_target_triplet.value.cmake_generator:
        msg.info_msg(
            "Using preferred {cmake_generator} CMake generator with the CMake configuration.".format(
                cmake_generator=json.dumps(alusus_target_triplet.value.cmake_generator)
            )
        )
    ...
    if not skip_installing_std_deps:
        ...
        for source_libname, dst_libname in libs_to_install:
            shlib_build_location = os.path.join(std_libs_location, source_libname)
        ...
        bins_to_install = []
        if (
            alusus_target_triplet.value.platform == "linux"
            or alusus_target_triplet.value.platform == "darwin"
        ):
            bins_to_install = [(os.path.join("llvm", "wasm-ld"), "wasm-ld")]
            # TODO: Add more bins to install as needed here.
        elif alusus_target_triplet.value.platform == "win32":
            bins_to_install = [(os.path.join("llvm", "wasm-ld.exe"), "wasm-ld.exe")]
            # TODO: Add more bins to install as needed here.
        # TODO: Add more logic for other targets here.
        ...
        for source_binname, dst_binname in bins_to_install:
            bin_build_location = os.path.join(std_bins_location, source_binname)
            ...
            if ret == common.CopyIfDifferentReturnType.DstUpdated:
                msg.success_msg(
                    "Installing STD binname={binname}.".format(
                        binname=json.dumps(dst_binname)
                    )
                )
            elif ret == common.CopyIfDifferentReturnType.DstUpToDate:
                msg.info_msg(
                    "STD binname={binname} is up to date.".format(
                        binname=json.dumps(dst_binname)
                    )
                )
            elif ret == common.CopyIfDifferentReturnType.NoSrc:
                msg.err_msg(
                    "STD binname={binname} doesn't exist.".format(
                        binname=json.dumps(dst_binname)
                    )
                )
...
def parse_cmd_args(args):
    ...
    parser.add_argument(
        "--build-location", type=str, default=None, help="Alusus build location"
    )
    parser.add_argument(
        "--install-location", type=str, default=None, help="Alusus install location"
    )
    parser.add_argument(
        "--build-type",
        type=BuildType.from_alusus_build_type_str,
        default=BuildType.DEBUG,
        help="Alusus build type",
    )
    parser.add_argument(
        "--target-triplet",
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Alusus built packages location",
    )
    parser.add_argument(
        "--bin-dirname",
        type=str,
    ...
    parser.add_argument(
        ...
        type=str,
        default="Include",
        help="Alusus include folder name inside Alusus install location",
    )
    parser.add_argument(
        "--force-cmake-generator", type=str, help="Use specific CMake generator"
    )
    parser.add_argument(
        "--skip-rpath", action="store_true", help="Don't use RPATH on supported systems"
    )
    parser.add_argument(
        "--skip-installing-std-deps",
        help="Whether or not to skip installing the standard libraries dependencies",
        action="store_true",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--print-supported-target-triplets",
        action="store_true",
        help="Prints the supported target triplets list and exits",
    )
    parser.add_argument(
        "--skip-build-tests", help="Do not build tests", action="store_true"
    )
    parser.add_argument(
        "--verbose",
        action="store_true",
    ...
    alusus_local_build_path = os.path.join(os.path.abspath(os.getcwd()), "AlususBuild")
